...
#define digitalWrite(gpiox, pin, state) if (state == HIGH ) {GPIO_WriteBit(gpiox, pin, Bit_SET); }else {GPIO_WriteBit(gpiox, pin, Bit_RESET);}

/*
 * Basic connection: only DIR, STEP are connected.
 * Microstepping controls should be hardwired.
 */
BasicStepperDriver::BasicStepperDriver(short steps, GPIO_TypeDef* dir_gpiox, short dir_pin, GPIO_TypeDef* step_gpiox, short step_pin)
:motor_steps(steps), dir_gpiox(dir_gpiox), dir_pin(dir_pin),  step_gpiox(step_gpiox), step_pin(step_pin)
{}

BasicStepperDriver::BasicStepperDriver(short steps, GPIO_TypeDef* dir_gpiox, short dir_pin, GPIO_TypeDef* step_gpiox, short step_pin, GPIO_TypeDef* enable_gpiox, short enable_pin)
:motor_steps(steps), dir_gpiox(dir_gpiox), dir_pin(dir_pin), step_gpiox(step_gpiox), step_pin(step_pin), enable_gpiox(enable_gpiox), enable_pin(enable_pin)
{}
...
void BasicStepperDriver::begin(short rpm, short microsteps){
	digitalWrite(dir_gpiox, dir_pin, HIGH);
...
void BasicStepperDriver::enable(void){
    if IS_CONNECTED(enable_pin){
        digitalWrite(enable_gpiox, enable_pin, LOW);
    }
}

void BasicStepperDriver::disable(void){
    if IS_CONNECTED(enable_pin){
        digitalWrite(enable_gpiox, enable_pin, HIGH);
    }
}
